...
LIMIT_VIDEO_MB = 50   # порог для reply_video (Telegram стримит как видео)
LIMIT_DOC_MB   = 2000 # максимум для reply_document (Bot API — 2 GB)
...
def compress_video(input_path: Path, output_path: Path, target_mb: int = 49) -> bool:
    """
    Сжимает видео через ffmpeg методом two-pass, чтобы уложиться в target_mb.
    ...
    # Целевой битрейт: оставляем 128 kbps на аудио
    target_bits = target_mb * 8 * 1024  # в kbits
    audio_kbps  = 128
...
def _download(url: str, ydl_opts: dict) -> dict | None:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(
        ...
        f"2. Если < {LIMIT_VIDEO_MB} MB — отправляет как видео (стримится прямо в чате)\n"
        f"3. Если > {LIMIT_VIDEO_MB} MB и есть ffmpeg — сжимает до {LIMIT_VIDEO_MB} MB\n"
        f"4. Если сжатие не помогло или ffmpeg нет — отправляет как файл (до {LIMIT_DOC_MB} MB)\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        try:
            ...
            # Случай 1: файл маленький — отправляем как видео
            ...
            # Случай 2: файл большой, пробуем сжать через ffmpeg
            if ffmpeg_available():
                ...
                success = await loop.run_in_executor(
                    None, lambda: compress_video(video_file, compressed)
                )
                ...
                if success and compressed.exists():
                    ...
                    if compressed_mb <= LIMIT_VIDEO_MB:
                        # Сжали успешно — отправляем как видео
                        await status_msg.edit_text(
                        ...
                        # Сжали, но всё равно большое — пойдём как документ
                        video_file = compressed
                        file_size_mb = compressed_mb
                else:
                    logger.warning("ffmpeg не смог сжать файл")

            # Случай 3: отправляем как документ (до 2 GB)
            if file_size_mb > LIMIT_DOC_MB:
                await status_msg.edit_text(
                    f"❌ Видео слишком большое ({file_size_mb:.0f} MB > {LIMIT_DOC_MB} MB). "
                    "Даже как файл не отправить."
                )
                return

            await status_msg.edit_text(
                f"📁 Видео {file_size_mb:.0f} MB — отправляю как файл\n"
                f"(воспроизводится после скачивания)"
            )
...
def main() -> None:
    if BOT_TOKEN == "ВСТАВЬ_ТОКЕН_СЮДА":
        raise ValueError(
            "Укажи токен бота! Либо через переменную окружения BOT_TOKEN, "
            "либо прямо в коде (переменная BOT_TOKEN)."
        )

    if not ffmpeg_available():
        logger.warning(
            "ffmpeg не найден! Сжатие больших видео недоступно. "
